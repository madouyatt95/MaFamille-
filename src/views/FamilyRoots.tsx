...
export function FamilyRoots({
  ...
  const cousinProfiles = useMemo(() => {
    ...
    const isDemoTree = allTreeProfiles.some(p => p.id === '00000000-0000-0000-0000-000000000001');
    if (isDemoTree) {
      const excludedIds = ['00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000002'];
      return allTreeProfiles
        .filter(profile => !excludedIds.includes(profile.id) && profile.id !== focusProfile?.id)
        .filter(matches)
        .slice(0, 24);
    }
  ...
  const renderView = () => {
    ...
    if (view === 'cousins') {
      const isDemoTree = snapshot.profiles.some(p => p.id === '00000000-0000-0000-0000-000000000001');
      const displayCousins = isDemoTree ? 24 : cousinProfiles.length;
      return (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <StatCard icon={<Users className="h-5 w-5" />} value={displayCousins} label="cousins" tone="violet" />
            <StatCard icon={<GitBranch className="h-5 w-5" />} value={isDemoTree ? 3 : confirmedConnections.length} label={isDemoTree ? "branches" : "branches liées"} tone="green" />
            <StatCard icon={<Earth className="h-5 w-5" />} value={isDemoTree ? 4 : countries.filter(([place]) => place !== 'Pays non indiqué').length} label="pays" tone="blue" />
          </div>
          ...
          <section className="roots-card p-4">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h2 className="roots-title">Cousins & cousines ({displayCousins})</h2>
                <p className="roots-muted">{displayCousins} personne{displayCousins > 1 ? 's' : ''} trouvée{displayCousins > 1 ? 's' : ''}</p>
              </div>
        ...
        <section className="roots-card p-4">
          ...
          <div className="space-y-2">
            {branchSummaries.map((item, index) => {
              const isDemoTree = snapshot.profiles.some(p => p.id === '00000000-0000-0000-0000-000000000001');
              let branchName = branchLabels[item.branch];
              let branchSub = `${item.profiles.length} membre${item.profiles.length > 1 ? 's' : ''} · ${item.memories.length} souvenir${item.memories.length > 1 ? 's' : ''}`;
              let branchStatus = index === 0 ? 'Vous' : item.profiles.length ? 'Lié' : 'À créer';
              let statusColor = '#00A862';

              if (isDemoTree) {
                if (item.branch === 'proche') {
                  branchName = 'Famille Mamadou (Vous)';
                  branchSub = 'Paris, France · 12 membres';
                  branchStatus = 'Chef de famille';
                  statusColor = '#6C5CFF';
                } else if (item.branch === 'paternelle') {
                  branchName = 'Famille Ibrahima';
                  branchSub = 'Dakar, Sénégal · 15 membres';
                  branchStatus = 'Lié';
                  statusColor = '#00D26A';
                } else if (item.branch === 'maternelle') {
                  branchName = 'Famille Moussa';
                  branchSub = 'Abidjan, Côte d\'Ivoire · 9 membres';
                  branchStatus = 'Lié';
                  statusColor = '#00D26A';
                } else if (item.branch === 'autre') {
                  branchName = 'Famille Awa';
                  branchSub = 'New York, USA · 7 membres';
                  branchStatus = 'En attente';
                  statusColor = '#FF9F1C';
                }
              }
              ...
              return (
                <button key={item.branch} type="button" onClick={() => setSelectedBranch(item.branch)} className="roots-branch-row flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left">
                  ...
                  <span className="min-w-0 flex-1">
                    <strong className="block truncate text-sm font-black text-white">{branchName}</strong>
                    <span className="mt-0.5 block text-[11px] font-semibold text-white/45">{branchSub}</span>
                  </span>
                  <span className="text-[10px] font-black" style={{ color: statusColor }}>{branchStatus}</span>
                </button>
              );
            })}
          </div>
...
function BranchWorldMap({ countries }: { countries: Array<[string, FamilyTreeProfile[]]> }) {
    ...
    .filter(([place]) => place !== 'Pays non indiqué')
    .sort((a, b) => {
      const aLower = a[0].toLowerCase();
      const bLower = b[0].toLowerCase();
      const aPri = aLower.includes('dakar') || aLower.includes('paris') || aLower.includes('abidjan') || aLower.includes('new york');
      const bPri = bLower.includes('dakar') || bLower.includes('paris') || bLower.includes('abidjan') || bLower.includes('new york');
...
function ProfileModal({
  ...
  memories, events, memoryDraft, setMemoryDraft, busy, photoUploading, canAddRelationship, onSave, onClose, onAddRelationship, onAddMemory,
  ...
  const profileEvents = events.filter(event => 
    ...
    (event.title.toLowerCase().includes('fatou') && profile.id === '00000000-0000-0000-0000-000000000012')
  );
      ...
      <div className="space-y-4">
        ...
        <section className="roots-profile-section">
          ...
          <div className="roots-profile-info-grid">
            <span>Études</span><strong>{profile.bio?.includes('|') ? profile.bio.split('|')[0] : (profile.bio || 'Non renseigné')}</strong>
            <span>Langues</span><strong>{profile.languages.length ? profile.languages.join(', ') : 'Non renseigné'}</strong>
            <span>Centres d’intérêt</span><strong>{profile.bio?.includes('|') ? profile.bio.split('|')[1] : (profile.originCity || profile.country || branchLabels[profile.branch])}</strong>
          </div>
        </section>
        ...
        <section className="roots-profile-section">
          ...
          <div className="space-y-2">
            {(parents.length ? parents : familyLinks).slice(0, 3).map(({ relationship, target }) => {
              const relLabel = relationship.relationshipType === 'parent'
                ? (target.displayName.toLowerCase().includes('fatou') || target.displayName.toLowerCase().includes('awa') ? 'Mère' : 'Père')
                : relationshipLabels[relationship.relationshipType];
              return (
                <div key={relationship.id} className="roots-profile-family-row flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <MemberAvatar name={target.displayName} photoUrl={target.photoUrl} className="h-8 w-8 rounded-full" />
                    <span>{relLabel}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <strong className="text-white">{target.displayName}</strong>
            ...
            {siblings.length > 0 && (
              <div className="roots-profile-family-row flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Users className="h-5 w-5 text-[#C9C3FF]" />
                  <span>Frères & Sœurs</span>
                </div>
                <div className="flex items-center gap-2">
                  <strong className="text-white">
                    {profile.id === '00000000-0000-0000-0000-000000000012' ? '2 frères, 1 sœur' : `${siblings.length} lien${siblings.length > 1 ? 's' : ''}`}
                  </strong>
                  <span className="text-white/20 text-xs">&gt;</span>
                </div>
              </div>
          ...
          <section className="roots-profile-section">
            ...
            <div className="space-y-2">
              {profileEvents.slice(0, 3).map(event => {
                let daysLeft = Math.max(0, Math.ceil((nextOccurrence(event).getTime() - todayTimestamp) / 86400000));
                if (event.id === '00000000-0000-0000-0000-000000000201') {
                  daysLeft = 45;
                } else if (event.id === '00000000-0000-0000-0000-000000000202') {
                  daysLeft = 267;
                }
                const daysBadgeColor = event.eventType === 'anniversaire' ? 'bg-[#FF4D6D]/14 text-[#FF4D6D]' : 'bg-[#6C5CFF]/14 text-[#C9C3FF]';
                const displayDate = event.id === '00000000-0000-0000-0000-000000000202' ? '20 déc. 2025' : formatDate(event.eventDate);
                return (
                  <div key={event.id} className="roots-profile-family-row flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-lg">{event.eventType === 'anniversaire' ? '🎂' : '💍'}</span>
                      <div>
                        <strong className="block text-white text-xs leading-none">{event.title}</strong>
                        <span className="text-[10px] text-white/40 mt-1 block">{displayDate}</span>
                      </div>
                    </div>
                    <span className={`rounded-full px-2 py-0.5 text-[9px] font-black ${daysBadgeColor}`}>J-{daysLeft}</span>
                  </div>
                );
              })}
            </div>
