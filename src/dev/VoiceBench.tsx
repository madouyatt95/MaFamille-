...
export function VoiceBench({ onTranscript, onListeningChange, compact = false, realProposal = false }: { onTranscript: (text: string, elapsed: number, alternatives?: string[]) => VoiceBenchObservation; onListeningChange: (active: boolean) => void; compact?: boolean; realProposal?: boolean }) {
  ...
  useEffect(() => { onListeningChange(listening && !(compact && phase === 'paused')); return () => onListeningChange(false); }, [listening, phase, compact, onListeningChange]);
  useEffect(() => {
  ...
  return <section className="mt-4 border-t border-family-border pt-4" aria-label="Banc d’essai vocal">
    ...
    <div className="mt-3 flex items-center gap-3">
      ...
      {listening && handsFree && phase === 'speaking' && <button title="Interrompre la lecture pour parler" aria-label="Interrompre la lecture pour parler" onClick={() => dialogue.current?.interrupt()} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-family-border"><Mic size={16} /></button>}
    ...
    {observation && !compact && <div className="mt-3 space-y-2 text-xs" aria-label="Mesures vocales">
