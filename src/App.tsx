...
const FOYER_TABLE_COLUMNS = {
  ...
  groceries: 'id, foyer_id, name, category, quantity, checked, in_stock, expiry_date, meal, added_by, is_favorite',
  archived_lists: 'id, foyer_id, name, date, items, store, created_by',
  dishes: 'id, foyer_id, day, meal_type, name, image, ingredients',
  chore_tasks: 'id, foyer_id, title, reward_points, assigned_member_id, assigned_member_name, done, rotation, validated_by_parent, due_date, reward_amount',
  saving_goals: 'id, foyer_id, title, target_amount, current_amount, target_date, category, contributions',
  alerts: 'id, foyer_id, title, description, time, type, read, module, sender_user_id, sender_member_id, sender_name, sender_avatar, created_at',
  votes: 'id, foyer_id, question, options, author_name, active, due_date',
  school_tasks: 'id, foyer_id, subject, title, due_date, done, assigned_member_id, difficulty, grade',
  ...
  pets: 'id, foyer_id, name, species, last_vaccine, next_vaccine, vet_appointment, notes, weight_history, document_ids',
  pocket_money: 'id, foyer_id, name, balance, points, avatar, shields, streak, last_shield_reset, last_connection, goal_title, goal_amount',
  artisans: 'id, foyer_id, name, specialty, phone, email, rating, notes',
  custom_categories: 'id, foyer_id, name, icon, color, budget, display_order',
  accounts: 'id, foyer_id, name, type, balance',
  abonnements: 'id, foyer_id, name, amount, period, next_billing_date, category',
  debts: 'id, foyer_id, title, amount, payer_id, payer_name, debtor_id, debtor_name, is_repaid',
  malus_templates: 'id, foyer_id, title, emoji, description, category, stars_removed, xp_removed, loss_streak, loss_shield, comment_required, double_parent_validation, created_at',
  malus_applied: 'id, foyer_id, member_id, title, emoji, description, stars_removed, xp_removed, loss_streak, loss_shield, comment, shield_used, repaired, repaired_at, reparation_task_id, created_at'
} as const;
...
function App() {
  ...
  const loadFoyerData = async (foyerId: string) => {
    ...
    const loadPromise = Promise.all([
      ...
      wrapQuery('events', client.from('events').select(FOYER_TABLE_COLUMNS.events).eq('foyer_id', foyerId)),
      wrapQuery('groceries', client.from('groceries').select(FOYER_TABLE_COLUMNS.groceries).eq('foyer_id', foyerId)),
      wrapQuery('archived_lists', client.from('archived_lists').select(FOYER_TABLE_COLUMNS.archived_lists).eq('foyer_id', foyerId)),
      ...
      wrapQuery('dishes', client.from('dishes').select(FOYER_TABLE_COLUMNS.dishes).eq('foyer_id', foyerId)),
      wrapQuery('chore_tasks', client.from('chore_tasks').select(FOYER_TABLE_COLUMNS.chore_tasks).eq('foyer_id', foyerId)),
      wrapQuery('saving_goals', client.from('saving_goals').select(FOYER_TABLE_COLUMNS.saving_goals).eq('foyer_id', foyerId)),
      wrapQuery('alerts', client.from('alerts').select(FOYER_TABLE_COLUMNS.alerts).eq('foyer_id', foyerId).order('created_at', { ascending: false }).limit(150)),
      ...
      wrapQuery('votes', client.from('votes').select(FOYER_TABLE_COLUMNS.votes).eq('foyer_id', foyerId)),
      wrapQuery('school_tasks', client.from('school_tasks').select(FOYER_TABLE_COLUMNS.school_tasks).eq('foyer_id', foyerId)),
      wrapQuery('chat_groups', client.from('chat_groups').select(FOYER_TABLE_COLUMNS.chat_groups).eq('foyer_id', foyerId)),
      wrapQuery('chat_messages', client.from('chat_messages').select(FOYER_TABLE_COLUMNS.chat_messages).eq('foyer_id', foyerId).order('created_at', { ascending: false }).limit(25)),
      wrapQuery('demarches', client.from('demarches').select(FOYER_TABLE_COLUMNS.demarches).eq('foyer_id', foyerId)),
      wrapQuery('justificatif_packs', client.from('justificatif_packs').select(FOYER_TABLE_COLUMNS.justificatif_packs).eq('foyer_id', foyerId)),
      wrapQuery('vehicles', client.from('vehicles').select(FOYER_TABLE_COLUMNS.vehicles).eq('foyer_id', foyerId)),
      wrapQuery('maintenance', client.from('maintenance').select(FOYER_TABLE_COLUMNS.maintenance).eq('foyer_id', foyerId)),
      wrapQuery('trips', client.from('trips').select(FOYER_TABLE_COLUMNS.trips).eq('foyer_id', foyerId)),
      wrapQuery('pets', client.from('pets').select(FOYER_TABLE_COLUMNS.pets).eq('foyer_id', foyerId)),
      wrapQuery('pocket_money', client.from('pocket_money').select(FOYER_TABLE_COLUMNS.pocket_money).eq('foyer_id', foyerId)),
      wrapQuery('artisans', client.from('artisans').select(FOYER_TABLE_COLUMNS.artisans).eq('foyer_id', foyerId)),
      wrapQuery('custom_categories', client.from('custom_categories').select(FOYER_TABLE_COLUMNS.custom_categories).eq('foyer_id', foyerId)),
      wrapQuery('accounts', client.from('accounts').select(FOYER_TABLE_COLUMNS.accounts).eq('foyer_id', foyerId)),
      wrapQuery('abonnements', client.from('abonnements').select(FOYER_TABLE_COLUMNS.abonnements).eq('foyer_id', foyerId)),
      wrapQuery('debts', client.from('debts').select(FOYER_TABLE_COLUMNS.debts).eq('foyer_id', foyerId)),
      wrapQuery('malus_templates', client.from('malus_templates').select(FOYER_TABLE_COLUMNS.malus_templates).eq('foyer_id', foyerId)),
      wrapQuery('malus_applied', client.from('malus_applied').select(FOYER_TABLE_COLUMNS.malus_applied).eq('foyer_id', foyerId))
    ]).then(([
      membersList,
      userRes,
      eventsRes,
  ...
  const syncDataFromCloud = async (foyerId: string) => {
    ...
    try {
      ...
      const [
        ...
        client.from('events').select(FOYER_TABLE_COLUMNS.events).eq('foyer_id', foyerId),
        client.from('groceries').select(FOYER_TABLE_COLUMNS.groceries).eq('foyer_id', foyerId),
        ...
        client.from('dishes').select(FOYER_TABLE_COLUMNS.dishes).eq('foyer_id', foyerId),
        client.from('chore_tasks').select(FOYER_TABLE_COLUMNS.chore_tasks).eq('foyer_id', foyerId),
        client.from('saving_goals').select(FOYER_TABLE_COLUMNS.saving_goals).eq('foyer_id', foyerId),
        ...
        client.from('votes').select(FOYER_TABLE_COLUMNS.votes).eq('foyer_id', foyerId),
        client.from('school_tasks').select(FOYER_TABLE_COLUMNS.school_tasks).eq('foyer_id', foyerId),
        client.from('chat_groups').select(FOYER_TABLE_COLUMNS.chat_groups).eq('foyer_id', foyerId),
        client.from('demarches').select(FOYER_TABLE_COLUMNS.demarches).eq('foyer_id', foyerId),
        client.from('justificatif_packs').select(FOYER_TABLE_COLUMNS.justificatif_packs).eq('foyer_id', foyerId),
        client.from('vehicles').select(FOYER_TABLE_COLUMNS.vehicles).eq('foyer_id', foyerId),
        client.from('maintenance').select(FOYER_TABLE_COLUMNS.maintenance).eq('foyer_id', foyerId),
        client.from('trips').select(FOYER_TABLE_COLUMNS.trips).eq('foyer_id', foyerId),
        client.from('pets').select(FOYER_TABLE_COLUMNS.pets).eq('foyer_id', foyerId),
        client.from('pocket_money').select(FOYER_TABLE_COLUMNS.pocket_money).eq('foyer_id', foyerId),
        client.from('artisans').select(FOYER_TABLE_COLUMNS.artisans).eq('foyer_id', foyerId),
        client.from('custom_categories').select(FOYER_TABLE_COLUMNS.custom_categories).eq('foyer_id', foyerId),
        client.from('accounts').select(FOYER_TABLE_COLUMNS.accounts).eq('foyer_id', foyerId),
        client.from('abonnements').select(FOYER_TABLE_COLUMNS.abonnements).eq('foyer_id', foyerId),
        client.from('debts').select(FOYER_TABLE_COLUMNS.debts).eq('foyer_id', foyerId)
      ]);
