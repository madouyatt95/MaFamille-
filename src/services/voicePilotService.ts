...
export type PilotSnapshot = { scope: string; foyerId: string; memberId: string; loadedAt: number; groceries: PilotGroceryRow[]; events: PilotEvent[]; external: Record<string, unknown>[] };
export async function loadVoicePilot(foyerId: string, memberId: string): Promise<PilotSnapshot> {
  ...
  return { scope: pilotScope(user.id, foyerId, memberId), foyerId, memberId, loadedAt: Date.now(), groceries: groceries.data, events: events.data, external: external.data };
}
