...
export const familyRootsService = {
  async load(foyerId: string, members: Member[], canManage: boolean): Promise<FamilyRootsSnapshot> {
    ...
    if (!client || !foyerId || foyerId === 'local') {
      return { profiles: fallbackProfiles, relationships: [], connections: [], identityRequests: [], events: generatedBirthdayEvents(fallbackProfiles), memories: [], corrections: [], validationLogs: [], cloudEnabled: false };
    }
    ...
    const existingRows = (ownProfilesResult.data || []) as ProfileRow[];
    if (canManage) {
